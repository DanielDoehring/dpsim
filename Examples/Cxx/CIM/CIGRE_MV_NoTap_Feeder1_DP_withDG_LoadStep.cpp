...
void addDGtoSystem(std::shared_ptr<CPS::Node<Complex>> node_DG,
	...
	if (withTrafo) {
		...
		trans_DG->setParameters(20e3 / Vnom_DG_node, 0., 0., lf_param);
...
namespace CPS {
	namespace SP {
		...
		void addDGtoSystem(std::shared_ptr<CPS::Node<Complex>> node_DG,
			...
			DistGen->setParameters(2 * PI * 50, Complex(Vnom_DG_node, 0),
				// controller parameters
				Pref, Qref, 0.25, 2, 0.001, 0.08, 3.77, 0.1,
				// filter parameters
		...
		SystemTopology calculatePowerFlow(String simName, Real system_freq,
			std::list<fs::path> filenames, Real penetrationLevel, std::vector<Real>& pvGenProfile) {
			...
			for (Int n = 3; n <= 11; n++) {
				...
				addDGtoSystem(node_DG, pv_at_node, "N" + std::to_string(n), "pv",
					Pref_pv_at_node, Qref_pv_at_node, systemSP, V_nom_DG, true);
			}
...
int main(int argc, char** argv){
	...
	String simName = "DP_faultN2_1e-3";
	...
	//pvGenProfile[0] = pvGenProfile[0] / 10;
	//pvGenProfile[1] = pvGenProfile[1] / 10;
	...
	CIM::Reader reader2(simName, Logger::Level::info, Logger::Level::info);
	...
	for (auto line : systemDP.mComponents) {
	...
	for (auto load : systemDP.mComponents) {
		if (shared_ptr<Ph1::RXLoad> load11 = std::dynamic_pointer_cast<Ph1::RXLoad>(load)) {
	...
	auto sw1 = SwitchEvent::make(17.0- timeStep  , fault_n2, true);
	auto sw2 = SwitchEvent::make(17.1 -timeStep , fault_n2, false);
