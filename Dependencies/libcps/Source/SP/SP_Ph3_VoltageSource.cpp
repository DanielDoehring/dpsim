...
PowerComponent<Complex>::Ptr SP::Ph3::VoltageSource::clone(String name) {
...
void SP::Ph3::VoltageSource::initializeFromPowerflow(Real frequency) {
...
/*
	...
	if (terminalNotGrounded(0)) {
		mLog.info() << "Add " << Complex(-1, 0) << " to " << simNode(0) << "," << mVirtualNodes[0]->simNode() << std::endl;
		mLog.info() << "Add " << Complex(-1, 0) << " to " << mVirtualNodes[0]->simNode() << "," << simNode(0) << std::endl;
	}
	if (terminalNotGrounded(1)) {
		mLog.info() << "Add " << Complex(1, 0) << " to " << mVirtualNodes[0]->simNode() << "," << simNode(1) << std::endl;
		mLog.info() << "Add " << Complex(1, 0) << " to " << simNode(1) << "," << mVirtualNodes[0]->simNode() << std::endl;
	}*/
}
